...
async function getRecipes() {
  ...
  // A1. COMPLETED - Check local storage to see if there are any recipes.
  //                 If there are recipes, return them.
  /**************************/
  const storedRecipes = localStorage.getItem('recipes');
  if (storedRecipes) {
    console.log('Loaded localStorage recipes');
  ...
  // A2. COMPLETED - Create an empty array to hold the recipes that you will fetch
  const fetchedRecipes = [];

  // A3. COMPLETED - Return a new Promise. If you are unfamiliar with promises, MDN
  ...
  return new Promise(async (resolve, reject) => {
     // A4. COMPLETED - Loop through each recipe in the RECIPE_URLS array constant
    //            declared above
    ...
    //            will be in the catch portion.
      //try get the recipe
      try{
        // A6. COMPLETED - For each URL in that array, fetch the URL - MDN also has a great
        ...
        let unparsedRecipe = await fetch(url);
        // A7. C - For each fetch response, retrieve the JSON from it using .json().
        //            NOTE: .json() is ALSO asynchronous, so you will need to use
        //            "await" again

        //parse the JSON
        let recipe = await unparsedRecipe.json();
        // A8. COMPLETED - Add the new recipe
        //add the recipe to the array
        fetchedRecipes.push(recipe);
      } catch (err) {
        // on error do the following
        console.error(`ERROR trying to fetch recipe ${i + 1}:`, err);
        // causese promise to be rejected
        reject(err);
        return;
      }
    }  
  // A9. COMPLETED - Check to see if you have finished retrieving all of the recipes,
  ...
  //            resolve() method.
    // Resolve promise . then save to storage
    saveRecipesToStorage(fetchedRecipes);
    console.log('🌐 Fetched and stored recipes from network');
